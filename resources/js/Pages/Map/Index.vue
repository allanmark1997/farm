...
const areaOfLand = (event) => {
    ...
    var polygon = leaflet
        .polygon(details.points, { color: details.color })
        .addTo(mymap);
    polygon
        .bindTooltip(details.address, { permanent: true, direction: "center" })
        .openTooltip();
    mymap.fitBounds(polygon.getBounds());
};
        ...
        <div class="py-12 grid grid-cols-8 px-2">
            <div class="bg-white col-span-2 p-4">
                <div class="flex flex-row-reverse gap-2">
                    <PrimaryButton @click="showAddCard = true"
                        >Add area</PrimaryButton
                    >
                    <PrimaryButton @click="downloadItem(formPDF)"
                        >Form</PrimaryButton
                    >
                </div>
                <div class="relative z-0 w-full mt-6 group border-none">
                    <select
                        class="truncate block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-ssr-blue2 border-[1.9px] appearance-none focus:outline-none focus:ring-0 peer rounded-lg px-[15px] disabled:cursor-not-allowed"
                        @change="areaOfLand($event)"
                    >
                    ...
                    <label
                        class="peer-focus:font-medium absolute text-sm text-gray-600 duration-300 transform -translate-y-6 scale-75 top-3 origin-[0] peer-focus:left-0 peer-focus:text-blue-600 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-85 peer-focus:-translate-y-6 ml-2 px-4 peer-placeholder-shown:-z-10 peer-focus:z-20 rounded-lg bg-white"
                        >Area of Land</label
                    >
                </div>

                <div
                    class="bg-slate-50 border-white shadow-lg rounded-lg my-2 p-2"
                >
                    <div>
                ...
                >
                    ...
                    <div class="my-4">
                        <div class="flex flex-col mt-1">
                            <select
                                class="truncate block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-ssr-blue2 border-[1.9px] appearance-none focus:outline-none focus:ring-0 peer rounded-lg px-[15px] disabled:cursor-not-allowed"
                                @change="form.farmer_id = $event.target.value"
                            >
                                <option selected="true" disabled="disabled">
                                    Farmers
                                </option>
                                <option
                                    v-for="(farmer, index) in props.farmers"
                                    :key="index"
                                    :value="farmer.id"
                                >
                                    {{ farmer.name }}
                        ...
                        <div class="flex gap-4 items-center mt-2">
                            ...
                            <input
                                type="color"
                                v-model="colorInput"
                                class="text-gray-900 rounded-md border-1 placeholder-gray-300 disabled:opacity-50 disabled:cursor-not-allowed disabled:bg-gray-700"
                            />
                        </div>
                        <PrimaryButton @click="handleConnect"
                            >Connect</PrimaryButton
                        >
                        <div class="flex flex-row-reverse gap-2">
                            <PrimaryButton @click="addArea">Save</PrimaryButton>
                            <SecondaryButton @click="cancelHandler"
