    ...
    <div class="mb-16">
        <div class="border-x-2 border-t-2 border-black mt-2">
            <div class="grid grid-cols-2">
                <div class=" flex items-center gap-2 pl-3 pb-2 grow">
                    ...
                    <div class="text-[12px] border-b border-black w-20 mr-2 pb-1">{{ farmers?.details?.householdLiving ||
                        'N/A' }}</div>
                </div>
                <div class=" pl-2 -mt-1 pt-1 grid grid-cols-4">
                    ...
                    <div class="text-xs pb-2">
                        <CheckTemplate :showCheck="farmers?.details.fourPs == 'Yes'" label="YES" />
                    </div>
                    <div class="text-xs pb-2">
                        <CheckTemplate :showCheck="farmers?.details.fourPs == 'No'" label="NO" />
                    </div>
                </div>
            </div>
                        ...
                        <tr>
                            ...
                            <th class="w-[20%] text-[10px] border-r-2 border-black px-2 pb-1">
                                <div class="font-black">
                                    CROP/COMMODITY <br><span
                                        class="inline italic text-[9px] font-normal">(Rice/Corn/HVC/Livestock/Poultry/<br />Agri-fishery)
                                    </span><br />
                                    For Livestock & Poultry<br /><span
                                        class="inline italic text[9px] font-normal pb-1">(specify type of animal)</span>
                                </div>
                            </th>
                            <th class="w-[10%] text-[10px] border-r-2 border-black">SIZE (ha)</th>
                            <th class="w-[10%] text-[10px] border-r-2 border-black">NO. OF HEAD <br /> <span
                                    class="italic text[9px]">(For Livestock and Poultry)</span></th>
                    ...
                    <tr v-for="(item, index) in parcel" :key="index">
                        ...
                        <td class="border-b border-black">
                            <div class="flex flex-col justify-start h-full ml-2 text-[10px]">
                                ...
                                <div class="border-b w-full box-border border-black h-3">{{ item.description?.barangay || ''
                                }}</div>
                                <div class="">
                                    Total Farm Areas:<span class="inline-block w-[full] border-b border-black w-12">{{
                                        item.description?.total_farm_area }}</span>ha
                                </div>
                                <div class="font-bold">*Ownership Document No <span
                                        class="inline-block w-[full] border-b border-black w-[58%] text-[8px]">{{
                                            item.description?.ownership_document_no }}</span></div>
                                <div class="flex">
                                    <div class="pb-2 grow">
                                        <CheckTemplate maxWidth="md" :sizeCheck="md"
                                            :showCheck="item.description?.farm_ownership == 'Registered Owner'"
                                            label="Registered Owner" />
                                    </div>
                                    <div class=" pb-2 grow">
                                        <CheckTemplate maxWidth="md" :sizeCheck="md"
                                            :showCheck="item.description?.farm_ownership == 'Others'" label="Others" />
                                    </div>
                                </div>
                                <div class="pb-2 ">
                                    <CheckTemplate maxWidth="md" :sizeCheck="md"
                                        :showCheck="item.description?.farm_ownership == 'Tenant'" label="Tenant" />
                                </div>
                                <div class=" ml-6 -mt-2">(Name of Land Owner: <span
                                        class="inline-block w-[full] border-b border-black w-[50%]">
                                        <p v-if="item.description?.farm_ownership == 'Tenant'">{{
                                            item.description?.farm_owner }}</p>
                                    </span>)</div>
                                <div class="pb-2 ">
                                    <CheckTemplate maxWidth="md" :sizeCheck="md"
                                        :showCheck="item.description?.farm_ownership == 'Lessee'" label="Lessee" />
                                </div>
                                <div class="ml-6 -mt-2">(Name of Land Owner: <span
                                        class="inline-block w-[full] border-b border-black w-[50%]">
                                        <p v-if="item.description?.farm_ownership == 'Lessee'">{{
                                            item.description?.farm_owner }}</p>
                                    </span>)</div>
                            </div>
                        </td>
                        <td class="border-b border-black">
                            <div class="flex flex-col p-0 h-[175px]">
                                <div class="h-6 border-b border-black grow lastCell" v-for="(crop, c) in item.crops"
                                    :key="c">
                                    <p class="text-sm mx-auto text-center">{{ (crop.details?.specified_animal == null) ? crop.details?.farm_type_business : crop.details?.specified_animal }}
                                    </p>
                                </div>
                            </div>
                        </td>
                        <td class="border-b border-black">
                            <div class="flex flex-col p-0 h-[175px]">
                                <div class="h-6 border-b border-black grow lastCell text-center  text-[1.5vmin]"
                                    v-for="(size, c) in item.sizes" :key="c">{{ size.details?.farm_size || '' }}</div>
                            </div>
                        </td>
                        <td class="border-b border-black">
                            <div class="flex flex-col p-0 h-[175px]">
                                <div class="h-6 border-b border-black grow lastCell text-center  text-[1.5vmin]"
                                    v-for="(head, c) in item.noOfHeads" :key="c">{{ head.details?.number_of_head_animal ||
                                        '' }}</div>
                            </div>
                        </td>
                        <td class="border-b border-black">
                            <div class="flex flex-col p-0 h-[175px]">
                                <div class="h-6 border-b border-black grow lastCell text-center text-[1.5vmin]"
                                    v-for="(farm, c) in item.farmTypes" :key="c">{{ farm.details?.farm_type || '' }}</div>
                            </div>
                        </td>
                        <td class="border-b border-black">
                            <div class="flex flex-col p-0 h-[175px] ">
                                <div class="h-6 border-b border-black grow lastCell text-center text-[1.5vmin]"
                                    v-for="(organic, c) in item.organics" :key="c">{{ organic.details?.organic_practitioner
                                        || '' }}</div>
                            </div>
                        </td>
                    </tr>
        ...
        <div class="border-x-2 border-b-2 border-black  py-2 text-[11px]">
            <p class="indent-4 pl-2">
                ...
                registration to the Registry System for Basic Sectors in Agriculture (RSBSA) and other legitimate interests
                of the Department pursuant to its mandates.
            </p>
        </div>
        ...
        <div class="border-x-2 border-b-2 border-black">
            ...
            <div class="text-[10px] pb-2">
                <p class="indent-4 pl-2">The collection of personal information is for documentation, planning, reporting
                    and processing purposes in availing agricultural related interventions.
                    Processed data shall only be shared to partner agencies for planning, reporting and other use in
                    accordance to the mandate of the agency. This is in
                    compliance with the Data Sharing Policy of the department.</p>
                <p class="indent-4 pl-2">
                    You have the right to ask for a copy of your personal data that we hold about you as well as to ask for
                    it to be corrected if you think it is wrong. To
        ...
        <div class="border-2 border-black h-36">
            ...
            <div class="grid grid-cols-3 h-[70%] gap-2 px-2 text-[8px] text-center items-end">
                <div class="border-t border-black">SIGNATURE ABOVE PRINTED NAME / DATE
                    <span class="block text-[10px] font-bold">BARANGAY CHAIRMAN</span>
                </div>
                <div class="border-t border-black">SIGNATURE ABOVE PRINTED NAME / DATE
